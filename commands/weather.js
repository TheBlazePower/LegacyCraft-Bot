...
module.exports.run = async (bot, message, args) => {
  ...
  weather.find({search: args.join(" "), degreeType: 'F'}, function(err, result) { // Make sure you get that args.join part, since it adds everything after weather.
      ...
      const embed = new Discord.RichEmbed()
          ...
          .setAuthor(`Cuaca Kota ${current.observationpoint}`) // This shows the current location of the weather.
          ...
          .addField('Zona Waktu:',`UTC${location.timezone}`, true) // This is the first field, it shows the timezone, and the true means `inline`, you can read more about this on the official discord.js documentation
          .addField('Jenis Derajat:',location.degreetype, true)// This is the field that shows the degree type, and is inline
          .addField('Suhu:',`${current.temperature} Degrees`, true)
          .addField('Terasa seperti:', `${current.feelslike} Degrees`, true)
